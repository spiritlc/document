...
<script>
  ...
  export default {
    ...
    mounted() {
      const changeLogNodes = this.$slots.default;
      console.log(changeLogNodes);
      let a = changeLogNodes[0].elm.querySelector('a');
      a && a.remove();
      let release = changeLogNodes[0].elm.innerText.trim();
      let fragments = `<li><h3><a href="https://github.com/ElemeFE/element/releases/tag/v${release}" target="_blank">${release}</a></h3>`;
      for (let len = changeLogNodes.length, i = 1; i < len; i++) {
        let node = changeLogNodes[i];
        if (node.elm.tagName !== 'H3') {
          fragments += changeLogNodes[i].elm.outerHTML || '';
        } else {
          release = changeLogNodes[i].elm.innerText.trim();
